...
/**
 * 대댓글 입력
 * 이벤트발생객체 e.target -> buton.btn-reply
 */
$(".btn-reply").click((e) => {
<% if(loginMember == null) { %>
	loginAlert();
...
<% } %> 
	...
	const tr = `<tr>
		<td colspan="2" stlye="text-align:left">
			<form 
			action="<%= request.getContextPath() %>/board/boardCommentEnroll" 
			method="post">
...
<% if(loginMember == null) { %>
	loginAlert();
//	e.preventDefault(); // 즉시중지는 아니기 때문에 이벤트의 기본행동만 막을 뿐 아래 코드는 실행된다.
//	return;
	return false; // 위에 두개 코드를 합친 효과
<% } %>
